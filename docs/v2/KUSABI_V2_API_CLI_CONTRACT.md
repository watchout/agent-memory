...
Status: frozen contract; the executable probe of §8 is **not yet relocated** and its pinned
baseline is stale — see the note in that section
Scope: canonical CLI registry, versioned envelopes, idempotency, the closed typed-error
registry and validation order, per-command payload/result contracts, alias boundary
Runtime impact: none
...
Origin: drafted in PR #253 against `docs/KUSABI_V2_API_CLI_CONTRACT.md`, outside the
canonical path. C1 requires the valid content to live in the canonical set instead; this
file is that relocation.

Draft provenance, retained: control source
[#180](https://github.com/watchout/agent-memory/issues/180), SPEC freeze
[comment 4975595110](https://github.com/watchout/agent-memory/issues/180#issuecomment-4975595110),
control handoff
[comment 4975612002](https://github.com/watchout/agent-memory/issues/180#issuecomment-4975612002),
...
> **Relocation note (2026-08-20, #304 C1).** The probe file and its fixture are still only
> in draft PR #253; this PR relocates the contract text alone. Do not read the command below
> as runnable on `main` yet. Two things must happen first, and they are their own cell
> because they touch `tests/**` and the test runner rather than `docs/v2/**`:
>
> 1. **Relocate** `tests/contracts/kusabi-v2-api-cli-contract.test.ts` (439 lines) and
>    `tests/contracts/fixtures/kusabi-v2-api-cli-contract.json` (320 lines), and wire them
>    into a runner. Nothing on `main` references `tests/contracts` today, so a file added
>    without wiring would be a probe that never runs — the appearance of a gate without one.
> 2. **Re-baseline** the exact-base evidence below. Both pinned digests are stale as of
>    `main` `f7fcd0f`:
>
>    | file | pinned in the probe | actual on `main` |
>    |---|---|---|
>    | `package.json` | `6264173c…` | `c7d4e9bf…` |
>    | `src/index.ts` | `789cda64…` | `910302d4…` |
>
>    Run unchanged, the probe would `BLOCK` because the baseline moved, not because the
>    contract failed. The classification conclusion is unaffected — the eight canonical
>    commands remain `not_implemented` — but the evidence must be re-pinned before the
>    result means anything.
>
> Until then this section states intent, not an available check.
