...
Scope: disposition of every legacy concept term, and the predicate that must hold
before any of them may be removed
Runtime impact: none
...
Origin: drafted in PR #252 against `docs/KUSABI_V2_LEGACY_CONCEPT_AUDIT.md`, outside the
canonical path. C1 requires the valid content to live in the canonical set instead; this
file is that relocation, with relative links rewritten for the new location. The draft's
control source was [#180](https://github.com/watchout/agent-memory/issues/180).

Relationship to the naming inventory: `KUSABI_V2_NAMING_SURFACE_INVENTORY.md` inventories
**surfaces** — package, CLI, MCP, environment and schema names — and the checkpoints for
changing them. This document classifies **concepts**: what each legacy term means in V2,
which canonical object it maps to, and what must be true before it can be removed. Consult
the inventory for a name, this audit for a term.
...
Both are relocated by the sibling C1 PRs; until those merge, the two links above resolve
only in the branch that adds each file.
...
[canonical v2 draft](./KUSABI_V2_CANONICAL_SPEC.md), the
[API/data boundary](./KUSABI_V2_API_AND_DATA_BOUNDARY.md), the
[v1 intent traceability record](./KUSABI_V2_V1_INTENT_TRACEABILITY.md),
