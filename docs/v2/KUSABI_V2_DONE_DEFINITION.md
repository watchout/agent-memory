...
Scope: the binary gates D01 to D14 that decide whether `v2_done` is true, and the
...
Origin: drafted in PR #254 against `docs/KUSABI_V2_DONE_DEFINITION.md`, outside the canonical
...
## 0. Relationship to the release claim ladder

`KUSABI_V2_RELEASE_CLAIM_LADDER.md` is already canonical and defines claim levels L0 to L4.
This document defines gates D01 to D14. They are layered, not competing:

- the **ladder** answers *which public claim may be asserted right now*, and what evidence
  each level costs;
- these **gates** answer *whether V2 itself is done*, which is a single binary over one
  frozen exact head.

The dependency runs one way. Gate D12 is written against the ladder — recovery quality must
meet "the claimed L0-L4 ladder evidence" — so the ladder supplies an input to this document,
and never the reverse. Reaching a ladder level does not make `v2_done` true, and `v2_done`
is not a claim level.

Where a term appears in both, the ladder governs claim vocabulary and this document governs
...
- release ladder: [`docs/v2/KUSABI_V2_RELEASE_CLAIM_LADDER.md`](./KUSABI_V2_RELEASE_CLAIM_LADDER.md)
- migration plan: [`docs/v2/KUSABI_V2_MIGRATION_PLAN.md`](./KUSABI_V2_MIGRATION_PLAN.md)
