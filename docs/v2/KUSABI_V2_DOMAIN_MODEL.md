...
Scope: the object model V2 is built from — identity, provenance, lifecycle,
mutation, redaction, ordering
Runtime impact: none
...
Origin: drafted in PR #252 against `docs/KUSABI_V2_DOMAIN_MODEL.md`, outside the canonical
path. C1 requires the valid content to live in the canonical set instead; this file is that
relocation. The original control source of the draft was
[#180](https://github.com/watchout/agent-memory/issues/180).

Companion documents from the same draft — the architecture and the legacy concept audit —
are separate PRs under C1 and are not linked here until they exist in `docs/v2/`.
...
- Missing or invalid promotion evidence means candidate, never implicit
  approval.
- When the authority is human, the reference is a `control_source_ref`
  (`{url, sha256}`) as frozen by the decision and claim citation contract in
  `KUSABI_V2_API_AND_DATA_BOUNDARY.md`. One vocabulary covers both surfaces: a
  promotion that cites a decision and a read that cites one use the same shape,
  and both fail closed when it is absent. `source_refs` and
  `promotion_evidence_refs` remain `string[]` evidence pointers and carry no
