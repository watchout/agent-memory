...
Scope: plane ownership, core boundary, ports and adapters, deterministic path,
state transitions, failure and recovery
Runtime impact: none
...
Origin: drafted in PR #252 against `docs/KUSABI_V2_ARCHITECTURE.md`, outside the canonical
path. C1 requires the valid content to live in the canonical set instead; this file is that
relocation. Relative links were rewritten accordingly — the document now sits beside the
files it references.

Draft provenance, retained: control source
[#180](https://github.com/watchout/agent-memory/issues/180), frozen requirements
[SPEC-KUSABI-001 freeze](https://github.com/watchout/agent-memory/issues/180#issuecomment-4975595110),
implementation handoff
[CH-SPEC-KUSABI-001-ARCH-DOMAIN-LEGACY-20260715](https://github.com/watchout/agent-memory/issues/180#issuecomment-4975612006),
exact design baseline `6e85144e4ec22f24d51cf1975c7d0448485df4b7`.

`KUSABI_V2_DOMAIN_MODEL.md` and `KUSABI_V2_LEGACY_CONCEPT_AUDIT.md` are relocated by the
sibling C1 PRs. Until those merge, the two links below resolve only in the branch that adds
each file.
...
[release claim ladder](./KUSABI_V2_RELEASE_CLAIM_LADDER.md) remains the only
...
- [existing API and data compatibility boundary](./KUSABI_V2_API_AND_DATA_BOUNDARY.md)
- [existing suite interop boundary](./KUSABI_V2_SUITE_INTEROP_BOUNDARY.md)
