...
| API and CLI contract | `docs/v2/KUSABI_V2_API_CLI_CONTRACT.md` |
...
| Typed protected personal data (2026-09-20 amendment) | `docs/design/typed-protected-memory-v1.md`, consuming the AUN-first shared contract |
...
| --- | --- | --- |
| `raw_event_source` | Imported transcript, host event, tool result, source reference, or raw evidence. | Data-only, provenance-bearing. New-mode eligible personal values use typed references and encrypted originals; current compatibility remains redacted. |
...
The user-directed 2026-09-20 amendment in `docs/design/typed-protected-memory-v1.md`
adds reversible personal-data protection with type, relation and uncertainty
metadata. It separates classification from authorization and excludes secrets
from the personal-data vault. AUN storage protection precedes Kusabi adaptation.
This design does not claim runtime implementation or independent acceptance.
...
The controlling backend parity planning record is
`KUSABI_V2_BACKEND_PARITY_MATRIX.md`. It defines SQLite, PostgreSQL, JSON, and
common DB claim boundaries and evidence classes. It does not change store
selection, fallback behavior, env vars, DB paths, migrations, fixtures, tests,
or authorize backend parity claims.
...
- backend parity matrix exists without storage behavior, fixture, migration, or parity-claim implementation;
