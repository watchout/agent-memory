...
Scope: the phased realization of V2 — freeze, canonical adapter, persistence,
consumer opt-in, sunset eligibility, removal template
Runtime impact: none

Control source: [#304](https://github.com/watchout/agent-memory/issues/304), CTO verdict
`CH-CTO-KUSABI-V2-DESIGN-VERDICT-20260817-002`, finding C1.
Origin: drafted in PR #254 against `docs/KUSABI_V2_MIGRATION_PLAN.md`, outside the canonical
path. C1 requires the valid content to live in the canonical set instead; this file is that
relocation.

Draft provenance, retained: cell `CELL-KUSABI-001-MIGRATION-DONE-CLAIM`, exact source head
`6e85144e4ec22f24d51cf1975c7d0448485df4b7`.

## 0. Relationship to the migration boundary

`KUSABI_V2_MIGRATION_BOUNDARY.md` is already canonical and already numbers phases 0 to 5.
This document numbers phases M0 to M5. **They are different models and must not be read as
one.** Two phase models in one design set is precisely the ambiguity finding C1 exists to
remove, so the division is stated here rather than left to the reader.

| | migration boundary | this plan |
|---|---|---|
| subject | **naming surfaces**: package, CLI, MCP, env, DB path, repository identity | **V2 realization**: adapters, persistence, consumers, evidence |
| asks | may this name change, and under what gate | is the V2 object model actually built and proven |
| decides | repository rename versus new repository, safe edit classes, default-name switch | per-backend proof, consumer cohorts, sunset eligibility |
| ends at | removal or legacy freeze of a **name** | removal template for an **alias surface**, after evidence |

Neither phase sequence gates the other. The boundary's phase numbers govern naming actions;
this plan's M-phases govern realization work. Where both speak about the same act — removing
a legacy alias — the stricter of the two applies, and in practice that is this plan's
section 7, because it requires consumer inventory, parity, warning period, isolation,
rollback and an owner sunset reference rather than owner approval alone.

Rough correspondence, for orientation only — it is not a mapping either document is bound
to: M0 presupposes boundary Phase 2's inventory; M1 realizes what boundary Phase 3 permits
additively; M4 evaluates what boundary Phase 4 would decide; M5 and boundary Phase 5 both
remain templates that grant nothing.
...
- existing claim ladder: [`docs/v2/KUSABI_V2_RELEASE_CLAIM_LADDER.md`](./KUSABI_V2_RELEASE_CLAIM_LADDER.md)
