# agent-memory v0.1.0-alpha MVP 実装仕様書 (HISTORICAL / SUPERSEDED)
...
> ## ⚠️ このドキュメントは歴史的記録です (2026-04-08 Arc退避)
>
> **現行実装 (v0.3.0) はこの仕様書と API/データモデルが乖離しています。**
>
> 2026-04-08 の CEO 判断により、**Path B (現行コードベースで進化)** を採用。現行 v0.3.0 に不足機能を加算する方針となりました。本仕様書は `sessions` テーブル + `memory_*` prefix ツール + sql.js default を前提とする **forward-looking な再設計プラン** ですが、現行実装は:
>
> - セッション概念なし（agent-comms session を流用）
> - ツール名は prefix なし（`log_decision`, `save_task_state` 等）
> - `knowledge` テーブル（`memories` ではない）、`source_type` / `source_ids` / `title` + `content` 分離
> - PostgreSQL デフォルト（sql.js は実装予定）
>
> **現行の single source of truth**: `docs/design/core/SSOT-*` ファイル群
>
> **MVP 到達計画**: `docs/IMPLEMENTATION_PLAN.md` を参照
>
> **本ドキュメントは歴史的記録として保持**されます。現行実装との照合や将来の `sessions` テーブル導入検討時の参考資料として活用してください。
