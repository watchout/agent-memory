...
| `task_continued` | recovery 後 10 分以内に bot が次の action (tool call) を実行したか | 未観測時は `NULL`。同一 agent/session/log に束縛された post-action 観測だけが `true` に更新する |
| `search_memory_count_10min` | boot 後 10 分以内の search_memory 呼出回数 | 低いほど復元品質が高い (search で補完する必要が無かった証拠) |
| `quality_score` | 0.0-1.0 のスコア | 未観測時は `NULL`。machine-observed continuation v1 は `1.0` を記録し、モデル名を `notes.quality_score_model` に必ず残す |
| `notes` | recovery summary と観測provenanceを含む JSON | post-action時は source、tool、observed_at、window_ms、quality_score_modelを追記する |

### 4.4 quality_score 算出仕様

現行の `binary_observed_continuation_v1` は、復旧後10分以内の最初の
eligible post-recovery state/action tool callをmachine observationとして `task_continued=true`、
`quality_score=1.0` にする。これは「行動継続を観測した」という二値スコアであり、
S1-S6の復旧内容評価やdefault-ready判定を置き換えない。異なる採点器が値を更新する
場合は、混同を避けるため採点モデルを `notes.quality_score_model` に記録する。

`recover_context`、`search_memory`、`get_decisions`、`get_knowledge`、
`restart_pack`、`restart_pack_fetch`、`restart_prepare` のような復旧・参照専用toolは
継続actionに数えない。これらはpending observationを保持し、後続のstate/action toolまたは
host verifierが結果を確定する。

boot/recover直後に `false` を書いてはならない。未観測は失敗ではないため `NULL` とし、
明示的なhost verifierが非継続を確定した場合にのみ `false` を記録する。
