...
Native Claude compatibility: the [official common input and SessionStart contract](https://code.claude.com/docs/en/hooks#common-input-fields) includes optional `scratchpad_dir` (2.1.257), `session_title`, resume/fork cache-age/token/cost metadata (2.1.251), and source `fork` (2.1.214). Accept their documented primitive types while ignoring them for identity, workspace and trust decisions; unsupported fields and malformed required identity still degrade explicitly. `fork` is normalized to the shared resume recovery operation while preserving the actual source in Claude evidence. This fixes a reproduced valid-shape parser rejection; it does not claim the exact nine historical warning payloads were captured.

SQLite native-event freshness (SC3-CA-01): each runtime-event read opens the current on-disk snapshot read-only; it never initializes, migrates or exports it. Closing a store never exports its old snapshot. Opening an already-current SQLite file also performs no persistence or write-lock acquisition when initialization makes no durable schema or data change. Fresh creation and actual schema migration, backfill or deduplication retain the same lock/CAS/atomic-replacement boundary. Initialization compares durable schema changes and value-changing migration DML, not exported-file bytes or matched-row counts from NULL-to-NULL assignments; FTS capability probes use a separate temporary database. Concurrent compiled MCP readers must initialize and read the registered native receipt while another process owns the write lock, preserving the database and lock bytes/identity; real writes and real migrations must still reject that lock. Reopening a migrated file must preserve its schema/data and become a no-write initialization. Every ordinary public store operation reads the current disk digest before its first query or mutation and reloads a changed snapshot without migration or export. Read/modify/persist segments do not yield or refresh midway; sequential interleaved stores therefore retain each other’s committed work without caller retries. Full-file persistence acquires an exclusive adjacent per-file lock, compares the disk digest to the loaded base and atomically replaces the file only if unchanged. A stale base fails with `SQLITE_STALE_SNAPSHOT`, discards the uncommitted local mutation and reloads current data; callers may explicitly retry. Lock contention fails immediately (`SQLITE_WRITE_LOCKED`), without spinning or erasing another owner’s lock. Release checks the creator's file identity and random owner token. Interrupted writes retain the last complete database; a crash-held lock remains fail-closed until its ownership/death is explicitly resolved, never automatically deleted from age alone. No arbitrary merge of memory rows, new table, migration or event registry is introduced. Tests keep a reader connected while a separate writer appends newer attempts, then exercise sequential interleaved operations, a writer overlapping after another operation loaded its base, close/reopen, contention and interrupted replacement: prior acceptance must never reappear and committed work must survive.
