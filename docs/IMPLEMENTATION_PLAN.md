...
| P0 | AM-039: selected restart pack fetch + boot consume | `restart_prepare` が選んだ pack を `selected_restart_pack:<id>` として保存し、MCP/CLI/boot で fetch または consume できるようにする |
...
### 2026-06-01 control-plane runner 方針更新 (#108)

AUN で整理した control-plane runner pattern を Wasurezu 側にも取り込む。
Wasurezu の session continuity / recovery / memory automation は、TUI 入力や
SessionStart self-kick を primary path にしない。canonical state は runtime
transcript ではなく Wasurezu の durable ledger とし、復旧判断は LLM prompt 内
判断ではなく scripts / hooks / runners / supervisor code が typed operation として
実行する。

この方針は既存の AM-040〜AM-043 に横断するため、次の P0 task を追加して
先に仕様・データモデルを固定する:

| Priority | Work | Purpose |
|----------|------|---------|
| P0 | #108 / AM-044: control-plane runner + durable lifecycle ledger | `SSOT-6` を最上位 continuity SSOT とし、`raw_events`, `session_checkpoints`, `recovery_packs`, `recovery_pack_items`, `session_lifecycle_events` を canonical state として整理 |
| P0 | AM-040 update: typed lifecycle bands/actions | `ok` / `prepare` / `warn` / `recommend` / `require` / `pack_only` / `on_demand` / `off` と `observe_context` / `prepare_restart` / `create_restart_pack` / `load_recovery_pack` / `record_recovery_result` / `recommend_restart` を API/runner 境界に入れる |
| P1 | AM-041 update: checkpoint + metrics source discipline | context percentage は host metrics がある時だけ使い、metrics と semantic degradation を `session_checkpoints` / lifecycle events に記録 |
| P1 | AM-042 update: AUN suite ownership contract | AUN suite mode では AUN が runtime restart / requeue / claim lifecycle を所有し、Wasurezu は pack/provenance/confidence/missing-context signals のみ提供 |
| P2 | AM-043 update: structured recovery evidence | every attempt で reason, owner, session_id, project, affected task/claim/goal, pack_id, provenance, confidence, missing_context, resumed/requeued/pending, full/partial/degraded/failed を記録 |

Acceptance:

- Wasurezu can produce a bounded, source-bearing restart pack without relying on TUI transcript scraping.
- Startup recovery reports confidence and missing context from deterministic runner state.
- Pre-exit prepare and post-start recovery are hook/runner paths, not prompt-driven decisions.
- Runtime adapters invoke the runtime, pass bounded context, and return structured evidence; they do not mutate lifecycle policy.
- AUN-supervised and Wasurezu-standalone ownership boundaries are explicit.
- TUI input is documented as compatibility fallback only.
- No prompt-driven context-limit decision is required for normal operation.
