...
## Company Dev OS / Kusabi #177 運用ルール

Canonical Company Dev OS model:
https://github.com/watchout/iyasaka-arc/issues/18

Kusabi/Wasurezu rollout:
https://github.com/watchout/agent-memory/issues/177

非 trivial な作業は、GitHub issue / PR を durable SSOT として扱う。AUN、
Discord、queue id、TUI 表示、ローカルメモは acceleration / projection であり、
完了根拠そのものではない。

memory / recovery 作業も bounded phase goal として実行する。各 phase は
Goal、Scope、Non-scope、Acceptance Criteria、Target files/modules、
Allowed actions、Required checks、Stop conditions、Evidence to write back to
GitHub、Next phase handoff を持つ。

作業開始時に runner policy を宣言または解決する:

- `codex_native_fast_lane`: Codex-safe な R0-R2 work。
- `claude_code_autonomous_lane`: Claude Code が active runtime の作業。
- `headless_runtime_adapter_lane`: memory/recovery evidence を保持できる adapter 経由のみ。
...
queue id、Discord projection、TUI visibility、green CI だけから推論しない。
memory capture、context recovery、restart pack、canonical
agent/project/workspace binding、必要に応じた live runtime evidence を GitHub
へ書き戻す。

Common DB alignment は https://github.com/watchout/agent-memory/issues/147
が継続 track。Kusabi/Wasurezu は memory/recovery product state を所有し、
common identity/runtime registry は common infrastructure として扱う。
