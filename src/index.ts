  ...
  server.tool(
    ...
    async ({ project }) => {
      ...
      try {
        ...
        if (proj) header.push(`Project: ${proj}`);
        header.push("");
        ...
        const bodyBudget = cfg.max_tokens - Math.ceil(headerText.length / 4) - Math.ceil(footerText.length / 4);
        const bodyParts = truncateByPriority(sections, Math.max(bodyBudget, 100));

        const output = [headerText, ...bodyParts, "", footerText].join("\n");
