...
export function buildRapidLiteReport(options) {
  ...
  const deliveryPlan = runDeliveryPlan({ resultDir, refs, actual });
  if (deliveryPlan) records.push(deliveryPlan);

  const designFlowG8 = runDesignFlowG8({ resultDir, refs });
  if (designFlowG8) records.push(designFlowG8);

  const transitionAdmission = runTransitionAdmission({ resultDir, refs, deliveryPlan });
  if (transitionAdmission) records.push(transitionAdmission);
  ...
  const controlState = runControlStateCompleteness({
    ...
    deliveryPlanReportPath: deliveryPlan?.status === "ran" ? deliveryPlan.output_path : refs.deliveryPlanReport,
    actual,
  });
...
function runControlStateCompleteness({
  ...
  addArg(args, "--delivery-plan", refs.deliveryPlan);
  addArg(args, "--delivery-plan-report", deliveryPlanReportPath);
  addArg(args, "--audit-checklist-report", auditChecklistReportPath);
  ...
  addArg(args, "--actual-pr", actual.actualPr);
  addArg(args, "--actual-head", actual.actualHead);
  addArg(args, "--actual-terminal-evidence-resolution", actual.actualTerminalEvidenceResolution);
  addArg(args, "--actual-terminal-evidence-comment-ref", actual.actualTerminalEvidenceCommentRef);
  args.push("--format", "json");
  return runGate({ gate: "control-state-completeness", args, outputPath: path.join(resultDir, "control-state-completeness.json") });
}

function runDeliveryPlan({ resultDir, refs, actual }) {
  if (!refs.deliveryPlan) return null;
  const args = [
    gateScript("validate-delivery-plan.mjs"),
    "--file",
    refs.deliveryPlan,
  ];
  addArg(args, "--actual-repository", actual.actualRepo);
  addArg(args, "--actual-commit-sha", actual.actualHead);
  addArg(args, "--actual-release-id", actual.actualReleaseId);
  addArg(args, "--actual-process-identity", actual.actualProcessIdentity);
  addArg(args, "--actual-config-digest", actual.actualConfigDigest);
  addArg(args, "--actual-policy-digest", actual.actualPolicyDigest);
  addArg(args, "--actual-environment", actual.actualEnvironment);
  addArg(args, "--actual-framework-version", actual.actualFrameworkVersion);
  addArg(args, "--actual-skill-digest", actual.actualSkillDigest);
  addArg(args, "--actual-schema-digest", actual.actualSchemaDigest);
  addArg(args, "--actual-validator-digest", actual.actualValidatorDigest);
  addArg(args, "--actual-terminal-evidence-resolution", actual.actualTerminalEvidenceResolution);
  addArg(args, "--actual-terminal-evidence-comment-ref", actual.actualTerminalEvidenceCommentRef);
  addArg(args, "--actual-pr", actual.actualPr);
  args.push("--format", "json");
  return runGate({ gate: "delivery-plan", args, outputPath: path.join(resultDir, "delivery-plan.json") });
}

function runDesignFlowG8({ resultDir, refs }) {
  if (!refs.cellGraph) return null;
  const args = [
    gateScript("check-design-flow-g8.mjs"),
    "--input",
    refs.cellGraph,
    "--format",
    "json",
  ];
  return runGate({ gate: "design-flow-g8", args, outputPath: path.join(resultDir, "design-flow-g8.json") });
}

function runTransitionAdmission({ resultDir, refs, deliveryPlan }) {
  if (!refs.transitionEvaluation) return null;
  if (!deliveryPlan || deliveryPlan.verdict !== "PASS" || deliveryPlan.would_block) {
    const outputPath = path.join(resultDir, "transition-admission.json");
    const report = {
      schema: "shirube-transition-admission-report/v1",
      verdict: "BLOCK",
      would_block: true,
      decision: "REJECT",
      reason_code: "PLAN_NOT_ADMITTED",
      queue_rows: 0,
      effects: 0,
      blockers: [{ severity: "BLOCK", code: "PLAN_NOT_ADMITTED", field: "delivery_plan", message: "The current DeliveryPlan dependency-closure, frontmost, WIP, self-pin, subject, and terminal-evidence gate must PASS before transition evaluation." }],
      required_next_actions: [{ item_id: "TRANSITION-PLAN-001", action: "Repair and re-run the current DeliveryPlan gate; do not create a queue row." }],
    };
    writeFileSync(outputPath, `${JSON.stringify(report, null, 2)}\n`);
    return gateRecordFromReport({ gate: "transition-admission", command: "DeliveryPlan prerequisite", outputPath, exitCode: 1, report });
  }
  const args = [
    gateScript("check-transition-admission.mjs"),
    "--file",
    refs.transitionEvaluation,
    "--format",
    "json",
  ];
  return runGate({ gate: "transition-admission", args, outputPath: path.join(resultDir, "transition-admission.json") });
}
...
function gateRecordFromReport({ gate, command, outputPath, exitCode, report }) {
  ...
  return {
    ...
    acceleration_decisions: report.acceleration_decisions ?? null,
    flow_recovery_receipts: report.flow_recovery_receipts ?? null,
...
function skipped(gate, reason) {
  return {
    ...
    head_change: null,
    disposition: null,
    acceleration_decisions: null,
    flow_recovery_receipts: null,
    would_block: false,
    blockers: [],
    warnings: [],
    required_next_actions: [],
...
function aggregateReport({ resultDir, refs, records, changedFiles }) {
  ...
  return {
    ...
    gates: outputRecords.map((record) => ({
      ...
      acceleration_decisions: record.acceleration_decisions,
      flow_recovery_receipts: record.flow_recovery_receipts,
...
function discoverRefs({ prBody, changedFiles }) {
  const explicit = {
    ...
    deliveryPlan: refFromBody(prBody, ["delivery_plan_ref", "delivery_plan", "delivery-plan"]),
    deliveryPlanReport: refFromBody(prBody, ["delivery_plan_report_ref", "delivery_plan_report", "delivery-plan-report"]),
    cellGraph: refFromBody(prBody, ["cell_graph_ref", "cell_graph", "cell-graph"]),
    transitionEvaluation: refFromBody(prBody, ["transition_evaluation_ref", "transition_evaluation", "transition-evaluation", "transition_admission_ref"]),
  ...
  const refs = {
    ...
    deliveryPlan: resolveRef({ name: "delivery_plan", explicit: explicit.deliveryPlan, candidates: bySchema(schemaMatches, "shirube-delivery-plan/v1").filter(notFixturePath), defaults: [".shirube/delivery-plan.yaml", ".shirube/self-dogfood/delivery-plan.yaml"], records }),
    deliveryPlanReport: resolveRef({ name: "delivery_plan_report", explicit: explicit.deliveryPlanReport, candidates: bySchema(schemaMatches, "shirube-delivery-plan-validation/v1").filter(notFixturePath), defaults: [".shirube/reports/delivery-plan.json"], records }),
    cellGraph: resolveRef({ name: "cell_graph", explicit: explicit.cellGraph, candidates: bySchema(schemaMatches, "shirube-cell-graph/v1").filter(notFixturePath), defaults: [], records }),
    transitionEvaluation: resolveRef({ name: "transition_evaluation", explicit: explicit.transitionEvaluation, candidates: bySchema(schemaMatches, "shirube-transition-evaluation/v1").filter(notFixturePath), defaults: [], records }),
...
function actualContextFromOptions(options) {
  ...
  return {
    ...
    actualReleaseId: stringOption(options["actual-release-id"]) ?? process.env.SHIRUBE_ACTUAL_RELEASE_ID ?? null,
    actualProcessIdentity: stringOption(options["actual-process-identity"]) ?? process.env.SHIRUBE_ACTUAL_PROCESS_IDENTITY ?? null,
    actualConfigDigest: stringOption(options["actual-config-digest"]) ?? process.env.SHIRUBE_ACTUAL_CONFIG_DIGEST ?? null,
    actualPolicyDigest: stringOption(options["actual-policy-digest"]) ?? process.env.SHIRUBE_ACTUAL_POLICY_DIGEST ?? null,
    actualEnvironment: stringOption(options["actual-environment"]) ?? process.env.SHIRUBE_ACTUAL_ENVIRONMENT ?? null,
    actualFrameworkVersion: stringOption(options["actual-framework-version"]) ?? process.env.SHIRUBE_ACTUAL_FRAMEWORK_VERSION ?? null,
    actualSkillDigest: stringOption(options["actual-skill-digest"]) ?? process.env.SHIRUBE_ACTUAL_SKILL_DIGEST ?? null,
    actualSchemaDigest: stringOption(options["actual-schema-digest"]) ?? process.env.SHIRUBE_ACTUAL_SCHEMA_DIGEST ?? null,
    actualValidatorDigest: stringOption(options["actual-validator-digest"]) ?? process.env.SHIRUBE_ACTUAL_VALIDATOR_DIGEST ?? null,
    actualTerminalEvidenceResolution: stringOption(options["actual-terminal-evidence-resolution"]) ?? process.env.SHIRUBE_ACTUAL_TERMINAL_EVIDENCE_RESOLUTION ?? null,
